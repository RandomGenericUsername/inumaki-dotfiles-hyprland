...
# Path to script that sets the variables of the current wallpper
set_current_wallpaper_script="{{cookiecutter.WALLPAPER_SELECTOR_SCRIPTS_DIR}}/set_current_wallpaper.sh"
...
# Source the set current wallpaper script
...
if [[ "$selected_wallpaper_name" != "$current_wallpaper_name" ]];then
    ...
    set_current_wallpaper "$selected_wallpaper"
    ...
    "$venv" update "{{cookiecutter.ROFI_CURRENT_WALLPAPER_EFFECT_VAR}}" "off" --env "{{cookiecutter.BASH_VENV}}"
