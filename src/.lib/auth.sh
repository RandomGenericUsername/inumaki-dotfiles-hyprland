...
auth() {
    local message=$1
    if [ "$(id -u)" -ne 0 ]; then
        print "$message" -t "info" -l "$LOG"
        sudo -k  # Force sudo to ask for a password again
        if sudo true; then
            print "Privileges acquired, continuing as root..." -t "info" -l "$LOG"
        else
            print "Failed to acquire necessary privileges" -t "error" -l "$LOG"
            exit 1
        fi
    fi
}
...
drop_root_privileges() {
    if [[ $EUID -ne 0 ]]; then
        print "Not running as root. No need to drop privileges." -t "debug" -l "$LOG"
        return 0  # Exit the function successfully if not running as root
    fi

    if [[ -z "$SUDO_USER" ]]; then
        print "Error: SUDO_USER is not set. Cannot drop root privileges." -t "debug" -l "$LOG"
        return 1  # Exit the function with an error status if SUDO_USER is not set
    fi

    print "Dropping root privileges, switching to user: $SUDO_USER" -t "debug" -l "$LOG"
